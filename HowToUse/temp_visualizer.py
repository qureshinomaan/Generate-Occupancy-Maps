...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, want_scores = True, want_boxes = True):
        """
        ...
        want = want_boxes and predictions.has("pred_boxes")
        boxes = predictions.pred_boxes if want else None
        want = want_scores and predictions.has("scores")
        scores = predictions.scores if want else None
    ...
    def draw_panoptic_seg_predictions(
        self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7, want_labels = True, want_scores = True, wanted_labels = []
    ):
        """
        ...
        for mask, sinfo in pred.semantic_masks():
            ...
            if text in wanted_labels or segment_some != True:
                if want_labels == False :
                    text = ''
                self.draw_binary_mask(
                    mask,
                    color=mask_color,
        ...
        try:
            ...
            colors = None
        self.overlay_instances(masks=masks, labels=labels, assigned_colors=colors, alpha=alpha, want_labels=want_labels, wanted_labels=wanted_labels)
    ...
    def overlay_instances(
        ...
        *,
        ...
        for i in range(num_instances):
            ...
            if masks is not None:
                if labels[i] in wanted_labels or wanted_seg == True:
            ...
            if labels is not None:
                ...
                if labels[i] in wanted_labels  and want_labels:
                    self.draw_text(
                        labels[i],
                        text_pos,
                        color=lighter_color,
